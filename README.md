...
- Проверить Claude slash commands, но переносить их только в skills, без создания slash-command shims.
- Проверить Claude subagents и предложить Codex custom agents, если роли реально нужны.
- Проверить global/project `MEMORY.md` и память: правила переносить в `AGENTS.md`/доки, а Codex Memories использовать только как локальный recall-layer.
- Проверить recent sessions: предпочитать официальный Codex import flow или делать summary выбранных сессий, не копировать transcript files оптом.
...
## Если Нужна Установка Как Skill

Основной сценарий — промпт выше. Установка как skill необязательна.

Если всё же хотите локально установленный skill:
...
После установки попросите агента использовать `ecosystem-sync` в нужном режиме:

```text
Используй ecosystem-sync в режиме audit.
Используй ecosystem-sync в режиме sync.
```
